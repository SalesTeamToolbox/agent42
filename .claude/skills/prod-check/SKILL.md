...
Run all six production health checks against the Frood server via SSH and produce a summary report.

## Prerequisites

The SSH config alias `frood-prod` must be configured. This connects to:
- **Server:** Contabo VPS at 163.245.217.2:2222
- **User:** `deploy`
- **Service:** `frood.service` (systemd)

**Before running any checks**, verify SSH connectivity:

```bash
ssh frood-prod "echo OK"
```

If this command fails or times out, STOP and tell the developer:

> SSH connection to `frood-prod` failed. Ensure your `~/.ssh/config` has an entry for `frood-prod` pointing to 163.245.217.2 on port 2222 with user `deploy` and a valid key.
...
```bash
ssh frood-prod "sudo systemctl status frood --no-pager -l"
```
...
```bash
ssh frood-prod "tail -50 ~/frood/frood.log"
```
...
```bash
ssh frood-prod "curl -s http://localhost:6333/healthz"
```
...
```bash
ssh frood-prod "curl -s -o /dev/null -w '%{http_code}' http://localhost:8000/api/health"
```
...
```bash
ssh frood-prod "df -h / /var/lib/qdrant"
```
...
|-------|--------|---------|
| Frood Service | OK/WARN/FAIL | [active/inactive, uptime, restarts] |
...
If DEGRADED or CRITICAL, list specific recommended actions (e.g., "Restart frood service", "Investigate Qdrant connection", "Clear disk space on /var/lib/qdrant").
